...
const Register = ({ clearTimerToScreenTwo, clientsList, qrCode }) => {

    const { setScreenIndex, waitingTimer } = useContent();
    const { count, start, stop, reset } = useCountDown(waitingTimer);
    //
    //
    useEffect(() => {
        if (count === 0) {
        ...
        <div className='flex flex-col h-full content-center  items-center text-4xl ml-8 animate-fadeIn'>
            <div className='w-2/3 m-8'>
                ...
                <div className='flex my-16 animate-dropIn' style={{ animationDelay: '200ms' }}>
                    <Image className="rounded-lg object-contain w-1/3" src={`/assets/img/qrCode${qrCode}.png`} alt="QR CODE" width={0} height={0} sizes='100' />
